...
const FAILURE_CLASSIFICATION_OPTIONS: Array<{ value: FailureClassification; label: string }> = [
  { value: "real_machine_failure", label: "Falha real da máquina" },
  { value: "electrical_failure", label: "Falha elétrica" },
  { value: "mechanical_failure", label: "Falha mecânica" },
  { value: "automation_sensor_failure", label: "Falha de automação / sensor" },
  { value: "operational_process_failure", label: "Falha operacional / processo" },
  { value: "quality_failure", label: "Falha de qualidade" },
  { value: "manual_intervention", label: "Intervenção manual" },
  { value: "simulation_test", label: "Simulação / teste" },
  { value: "unidentified_stop", label: "Parada sem causa identificada" },
  { value: "other", label: "Outro" },
];

function getFailureClassificationFromLegacy(source?: string | null): FailureClassification {
  if (source === "manual_simulation") return "simulation_test";
  if (source === "manual") return "manual_intervention";
  return "unidentified_stop";
}

function getFailureClassificationLabel(classification?: FailureClassification, source?: string | null) {
  const value = classification ?? getFailureClassificationFromLegacy(source);
  return FAILURE_CLASSIFICATION_OPTIONS.find((item) => item.value === value)?.label ?? "Parada sem causa identificada";
}
...
export function MachineFailureHistoryPage({ machineId }: MachineFailureHistoryPageProps) {
  ...
  const [editingClassification, setEditingClassification] = useState<FailureClassification>("unidentified_stop");
      ...
      {history.length===0 ? <EmptyState icon={<FileWarning className="h-12 w-12" />} title="Sem falhas registradas" description="Quando houver falhas para esta máquina, elas aparecerão aqui." /> : <div className="space-y-2.5">{history.map((event)=>{
        ...
        return <article key={event.id} className="rounded-xl border border-border bg-card p-4">
          <dl className="grid grid-cols-1 gap-x-3 gap-y-2 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div><dt className="text-xs uppercase text-muted-foreground">Classificação da ocorrência</dt><dd className="font-bold">{getFailureClassificationLabel(event.failureClassification, event.source)}</dd></div>
            ...
            <div className="sm:col-span-2 lg:col-span-3"><dt className="text-xs uppercase text-muted-foreground">Descrição da ocorrência</dt><dd className="text-foreground">{editingId===event.id ? <div className="flex flex-col gap-2"><label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Classificação da ocorrência</label><select className="h-10 rounded-xl border border-border bg-background px-3 text-sm" value={editingClassification} onChange={(e)=>setEditingClassification(e.target.value as FailureClassification)}>{FAILURE_CLASSIFICATION_OPTIONS.map((option)=><option key={option.value} value={option.value}>{option.label}</option>)}</select><label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Descrição da ocorrência</label><textarea className="min-h-[88px] rounded-xl border border-border bg-background p-3 text-sm" value={editingText} onChange={(e)=>setEditingText(e.target.value)} /><div className="flex gap-2"><button type="button" onClick={()=>{updateMachineStopEventDescription(machine.id,event.id,editingText.trim(),editingClassification);setEditingId(null);}} className="inline-flex items-center gap-2 self-start rounded-xl bg-secondary px-3 py-2 text-xs font-bold uppercase tracking-wider text-secondary-foreground"><Save className="h-4 w-4" />Salvar</button><button type="button" onClick={()=>{setEditingId(null);}} className="inline-flex items-center gap-2 self-start rounded-xl border border-border px-3 py-2 text-xs font-bold uppercase tracking-wider">Cancelar</button></div></div> : <div className="flex items-start justify-between gap-3"><span>{event.failureDescription || "Sem descrição"}</span><button type="button" onClick={()=>{setEditingId(event.id);setEditingText(event.failureDescription || "");setEditingClassification(event.failureClassification ?? getFailureClassificationFromLegacy(event.source));}} className="inline-flex items-center gap-1 rounded-lg border border-border px-2 py-1 text-xs font-bold uppercase tracking-wider"><Pencil className="h-4 w-4" />Editar</button></div>}</dd></div>
          </dl>
