  ...
  return <div className="space-y-3">
    ...
    {machineCalls.length === 0 ? <EmptyState icon={<History className="h-12 w-12" />} title="Sem chamados registrados" description="Quando houver chamados para esta máquina, eles aparecerão aqui." /> :
      <div className="space-y-2">{machineCalls.map((call) => {
        ...
        const impactBreakdown = calculateOperationalImpactBreakdown({
          ...
          readyFrom:
            call.maintenanceCompletedAt && (
              call.machineStatusAtFinish === "running" ||
              call.machineStatusAtAttend === "running" ||
              call.machineCondition === "running"
            )
              ? call.maintenanceCompletedAt
              : null,
          now,
        ...
        const allocations = Array.isArray(call.technicianTimeAllocations) ? call.technicianTimeAllocations : [];
        const finalTechnicianNames = call.technicianNames.length > 0
