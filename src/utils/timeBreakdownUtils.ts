...
function getMachineConditionAtTimestamp(
  ...
  if (relevantStopEvents.length === 0) {
    return fallbackMachineCondition === "stopped" ? "stopped" : "running";
  }
  ...
  return isInsideStopEvent ? "stopped" : "running";
}
...
function getProductionModeAtTimestamp(
  ...
  if (relevantProductionEvents.length === 0) {
    return fallbackMode;
  }
...
function finalizeOperationalImpactSeconds(
  ...
  classifiedSeconds = OPERATIONAL_IMPACT_CATEGORIES.reduce((total, category) => total + result[category], 0);
  if (classifiedSeconds > totalSeconds) {
    let overflow = classifiedSeconds - totalSeconds;
    for (const category of OPERATIONAL_IMPACT_CATEGORIES.slice().reverse()) {
      const removable = Math.min(result[category], overflow);
      result[category] -= removable;
      overflow -= removable;
      if (overflow === 0) break;
    }
  }
...
export function calculateOperationalImpactBreakdown(params: OperationalImpactParams): OperationalImpactBreakdown {
  ...
  for (let index = 0; index < sortedBoundaries.length - 1; index += 1) {
    ...
    if (isStopped && isScheduled) {
      categoryMilliseconds.productiveDowntimeSeconds += segmentMs;
    } else if (isStopped && !isScheduled) {
      categoryMilliseconds.nonScheduledDowntimeSeconds += segmentMs;
    } else if (!isStopped && isScheduled) {
      categoryMilliseconds.productionBlockedSupportSeconds += segmentMs;
    } else {
      categoryMilliseconds.nonScheduledSupportSeconds += segmentMs;
    }
  }
