...
function getMachineConditionAtTimestamp(
  ...
  const readyFromDate = toValidDate(readyFrom ?? null);
  const readyFromMs = readyFromDate?.getTime();
  ...
  const isInsideStopEvent = relevantStopEvents.some((event) => {
    ...
    const eventEnd = toValidDate(event.resumedAt ?? null) ?? now;
    let eventEndMs = eventEnd.getTime();

    // Se a manutenção concluiu como "pronta para rodar", encerra logicamente
    // a parada original nesse ponto para o cálculo de impacto operacional.
    if (readyFromMs && readyFromMs > eventStartMs && eventStartMs < readyFromMs) {
      eventEndMs = Math.min(eventEndMs, readyFromMs);
    }

    return timestamp >= eventStartMs && timestamp < eventEndMs;
  });

  if (isInsideStopEvent) return "stopped";

  if (readyFromMs && timestamp >= readyFromMs) return "running";
...
export function calculateOperationalImpactBreakdown(params: OperationalImpactParams): OperationalImpactBreakdown {
  ...
  const readyFromDate = toValidDate(params.readyFrom ?? null);

  const boundaries = new Set<number>([bounds.start, bounds.end]);

  if (readyFromDate) {
    clampBoundary(boundaries, readyFromDate.getTime(), bounds.start, bounds.end);
  }
