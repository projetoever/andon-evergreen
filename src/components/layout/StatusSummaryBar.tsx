...
function StatCard({ label, value, subtitle, tone, icon, pulse }: StatCardProps) {
  return (
    <div
      className={cn(
        "flex flex-1 items-center gap-2.5 rounded-lg border border-border bg-card px-3 py-2 shadow-sm",
        pulse && value > 0 && "ring-2 ring-danger animate-andon-pulse",
      )}
    >
      <div className={cn("flex h-9 w-9 items-center justify-center rounded-md md:h-10 md:w-10", toneBg[tone])}>
        {icon}
      </div>
      <div className="flex min-w-0 flex-col">
        <span className="text-[11px] uppercase tracking-wide text-muted-foreground">{label}</span>
        {subtitle && <span className="text-[11px] text-muted-foreground">{subtitle}</span>}
        <span className="text-xl font-bold tabular-nums leading-tight text-foreground md:text-2xl">{value}</span>
      </div>
    </div>
  );
}
