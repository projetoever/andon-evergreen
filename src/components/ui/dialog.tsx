...
import { VIRTUAL_KEYBOARD_OPEN_ATTRIBUTE } from "@/utils/virtualKeyboardUtils";
      ...
      <DialogPortal>
        ...
        <DialogPrimitive.Content
          ...
          onEscapeKeyDown={(event) => {
            if (!preventParentDismiss(event)) onEscapeKeyDown?.(event);
          }}
          onPointerDownOutside={(event) => {
            if (!preventParentDismiss(event)) onPointerDownOutside?.(event);
          }}
          onFocusOutside={(event) => {
            if (!preventParentDismiss(event)) onFocusOutside?.(event);
          }}
          onInteractOutside={(event) => {
            if (!preventParentDismiss(event)) onInteractOutside?.(event);
          }}
          {...props}
        >
          {children}
